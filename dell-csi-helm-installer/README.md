...
NOTE: This documentation uses the vcsi driver as an example. If working with a different driver, substitute the name as appropriate.
...
For example, to create a values file for the vCSI driver the following steps can be executed
```
# cd to  the installation script directory
cd dell-csi-helm-installer

# copy the template file
cp ../helm/vcsi-driver/values.yaml ./my-vcsi-driver-settings.yaml

# edit the newly created values file
vi my-vcsi-driver-settings.yaml
```
...
Installing a driver is performed via the `csi-install.sh` script. This script requires a few arguments: the target namespace and the user created values file. By default, this will verify the Kubernetes environment and present a list of warnings and/or errors. Errors must be addressed before installing, warning should be examined for their applicability. For example, in order to install the vCSI driver into a namespace called "vcsidriver", the following command should be run:
```
./csi-install.sh --namespace vcsidriver --values ./my-vcsi-driver-settings.yaml
```
...
Upgrading a driver is very similar to installation. The `csi-install.sh` script is run, with the same required arguments, along with a `--upgrade` argument. For example, to upgrade the previously installed vCSI driver, the following command can be supplied:

```
./csi-install.sh --namespace vcsidriver --values ./my-vcsi-driver-settings.yaml --upgrade
```
...
To uninstall a driver, the `csi-uninstall.sh` script provides a handy wrapper around the `helm` utility. The only required argument for uninstallation is the namespace name. To uninstall the vCSI driver:

```
./csi-uninstall.sh --namespace vcsidriver
```
...
```
./verify.sh --namespace vcsidriver --values ./my-vcsi-driver-settings.yaml
```
